...
def analyze_smci_strategy_yearly():
    """Detailed year-by-year breakdown of SMCI strategy performance"""
    
    # Load the detailed trades
    trades_df = pd.read_csv('smci_relaxed_trades.csv')
    
    # Load buy & hold analysis
    buy_hold_df = pd.read_csv('smci_yearly_buy_hold_analysis.csv')
    
    print("SMCI Strategy vs Buy & Hold: Detailed Year-by-Year Comparison")
    print("=" * 90)
    
    # Group trades by year
    yearly_strategy = trades_df.groupby('year').agg({
        'return_pct': ['count', 'sum', 'mean', lambda x: (x > 0).mean() * 100],
        'action': lambda x: (x == 'double_down').sum()
    }).round(2)
    
    yearly_strategy.columns = ['trades', 'total_return_raw', 'avg_return', 'win_rate', 'double_downs']
    
    # Calculate compound returns properly
    compound_returns = {}
    for year in trades_df['year'].unique():
        ...
        compound_returns[year] = (compound_return - 1) * 100
    ...
    for year in sorted(trades_df['year'].unique()):
        ...
        strategy_return = compound_returns[year]
        
        # Buy & hold performance
        bh_row = buy_hold_df[buy_hold_df['year'] == year]
        buy_hold_return = bh_row['yearly_return'].iloc[0] if len(bh_row) > 0 else 0
    ...
    # Overall totals
    total_strategy_compound = 1
    total_bh_compound = 1
    
    for year in sorted(trades_df['year'].unique()):
        total_strategy_compound *= (1 + compound_returns[year] / 100)
        bh_row = buy_hold_df[buy_hold_df['year'] == year]
        if len(bh_row) > 0:
            total_bh_compound *= (1 + bh_row['yearly_return'].iloc[0] / 100)
    
    total_strategy = (total_strategy_compound - 1) * 100
    total_bh = (total_bh_compound - 1) * 100
    total_diff = total_strategy - total_bh
    
    print(f"{'TOTAL':<6} {total_strategy:+11.2f}% {total_bh:+11.2f}% {total_diff:+11.2f}% "
    ...
    for _, trade in trades_df.iterrows():
        ...
        comment = ""
        if trade['return_pct'] > 25:
            comment = "🚀 HUGE WIN"
        elif trade['return_pct'] > 10:
            comment = "✅ Great"
        elif trade['return_pct'] < -10:
            comment = "💀 Big Loss"
        elif trade['return_pct'] < -5:
            comment = "⚠️ Loss"
    ...
    for year in sorted(trades_df['year'].unique()):
        ...
        if year == 2022:
            print(f"  📈 Good start: RSI filtering worked well in volatile market")
        elif year == 2023:
            print(f"  🚀 Excellent year: Strategy captured good momentum, but B&H had AI boom")
        elif year == 2024:
            print(f"  📉 Tough year: Both strategies struggled, but strategy limited losses")
        elif year == 2025:
            print(f"  🔄 Recovery: Strategy staying positive while B&H turned negative")
    ...
    strategy_returns = [compound_returns[year] for year in sorted(compound_returns.keys())]
    bh_returns = [buy_hold_df[buy_hold_df['year'] == year]['yearly_return'].iloc[0] 
                  for year in sorted(compound_returns.keys())]
    ...
    print(f"Strategy is {bh_std/strategy_std:.1f}x more consistent" if strategy_std < bh_std else f"Buy & Hold is {strategy_std/bh_std:.1f}x more consistent")
