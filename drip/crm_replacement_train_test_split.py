...
def test_3rd_4th_monday_strategy_split(df, ticker, period_type="training"):
    """Test 3rd→4th Monday strategy on ticker data with train/test split"""
...
def analyze_single_ticker(ticker):
    """Analyze a single ticker with train/test split"""
    try:
        ...
        training_result = test_3rd_4th_monday_strategy_split(df, ticker, "training")
        
        # Test testing period (June-Aug)  
        testing_result = test_3rd_4th_monday_strategy_split(df, ticker, "testing")
...
def find_best_crm_replacement_train_test():
    """Find the best CRM replacement using train/test split methodology"""
    
    print("🚀 CRM REPLACEMENT SELECTION - TRAIN/TEST SPLIT METHODOLOGY")
    print("=" * 90)
    print("📚 TRAINING PERIOD: January - May 2025")
    print("🧪 TESTING PERIOD: June - August 2025")
    print("🎯 STRATEGY: 3rd Monday → 4th Monday")
    print("=" * 90)
    ...
    best_replacement = final_ranking[0]
    ...
    # Compare vs CRM
    crm_return = -0.83  # From previous analysis
    improvement = best_replacement['testing_return'] - crm_return
    
    print(f"📊 PERFORMANCE vs CRM:")
    print("-" * 40)
    print(f"CRM (3rd→4th Monday):     {crm_return:+7.1f}%")
    print(f"{best_replacement['ticker']} (3rd→4th Monday): {best_replacement['testing_return']:+7.1f}%")
    print(f"Improvement:              {improvement:+7.1f} percentage points")
    ...
    results_df.to_csv('crm_replacement_train_test_results.csv', index=False)
    print(f"✅ Full results: 'crm_replacement_train_test_results.csv'")
    
    # Save winner's detailed trades
    winner_result = best_replacement['full_result']
    
    # Training trades
    training_trades = winner_result['training']['trades']
    training_trades['period'] = 'Training'
    training_filename = f"{best_replacement['ticker'].lower()}_training_trades.csv"
    ...
    testing_filename = f"{best_replacement['ticker'].lower()}_testing_trades.csv"
    ...
    combined_filename = f"{best_replacement['ticker'].lower()}_all_trades.csv"
    combined_trades.to_csv(combined_filename, index=False)
    
    print(f"✅ Winner trades: '{combined_filename}'")
    
    # Final recommendation
    print(f"\n🎯 FINAL RECOMMENDATION:")
    print("=" * 50)
    print(f"Replace CRM with {best_replacement['ticker']} in the weekly pipeline")
    print(f"Expected improvement: {improvement:+.1f} percentage points per 3-month cycle")
    print(f"Strategy validation: Passed out-of-sample testing")
    print(f"Risk profile: {winner_result['testing']['volatility']:.1f}% volatility")
    
    return best_replacement

if __name__ == "__main__":
    find_best_crm_replacement_train_test()