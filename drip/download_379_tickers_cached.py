#!/usr/bin/env python3
"""
Download Fed rate cut data for 379 tickers using caching system
Enhanced with key rate-sensitive sectors: TLT, VIX, mortgage REITs, homebuilders
"""
...
def download_ticker_data(ticker):
    ...
    try:
        ...
        if not data.empty and len(data) > 150:  # More lenient for some ETFs
            # Save to cache
...
def main():
    # Read enhanced 379-ticker list
    try:
        with open('fed_strategy_400_tickers.txt', 'r') as f:
            tickers = f.read().strip().split(',')
    except:
        print("Error: Could not load enhanced 379-ticker list")
        print("Make sure you've run create_400_tickers_with_rate_sensitive.py")
        return
    
    print(f"ENHANCED FED RATE CUT DATA DOWNLOAD")
    print("=" * 60)
    print(f"Tickers to process: {len(tickers)} (enhanced with rate-sensitive)")
    print("Period: Dec 1, 2018 - Dec 31, 2019")
    print("Includes: TLT, VIX plays, mortgage REITs, homebuilders, regional banks")
    ...
    for i, ticker in enumerate(tickers, 1):
        ...
        if not was_cached:
            time.sleep(0.02)  # Faster for enhanced dataset
    ...
    # Create DataFrame
    if all_rows:
        print(f"\nCreating enhanced dataset with {len(all_rows):,} rows...")
        ...
        # Save enhanced data
        filename = f'fed_{len(df["Ticker"].unique())}_tickers_enhanced_clean.csv'
        ...
        # Show key rate-sensitive additions
        key_additions = ['TLT', 'UVXY', 'NLY', 'AGNC', 'DHI', 'LEN', 'RF', 'FITB']
        found_additions = [t for t in key_additions if t in df['Ticker'].unique()]
        print(f"\nKey rate-sensitive additions found: {found_additions}")
        
        print(f"\n" + "=" * 60)
        print(f"SUCCESS! Enhanced {len(df['Ticker'].unique())}-ticker Fed strategy dataset ready!")
        print("=" * 60)
        print("Includes critical rate-sensitive sectors:")
        print("  - Treasury bonds (TLT, SHY, TIP)")
        print("  - Volatility (UVXY, VXX, VIXY)")
        print("  - Mortgage REITs (NLY, AGNC, STWD)")
        print("  - Homebuilders (DHI, LEN, PHM)")
        print("  - Regional banks (RF, FITB, HBAN)")
        print("Next: Run enhanced Fed rate cut strategy analysis!")
